/**
 * Created by huanghuang on 2017/3/6.
 */
...
/**
* 判断是否为中文、英文、字母或_  ^[_0-9a-zA-Z\u4e00-\u9fa5]+$
* 6个汉字或12个字母以内
* @param strValue 校验的值
* @returns {boolean}
*/
function isCnAndEnNumeric(strValue) {
var pattern = /^([\u4e00-\u9fa5]{1,6}|[a-zA-Z\.\s]{1,12})$/;
return executeExp(pattern, strValue);
}
...
/*
* 判断是否是字母数字下划线任意组合 4~20位
* */
function isEnNumericLimit(strValue) {
var pattern = /^([_0-9a-zA-Z]{4,20})$/;
return executeExp(pattern, strValue);
}
//首位必须是字母，由字母和数字组合 2~32位
function isEnNumerFirstEn(strValue) {
var pattern = /^([a-zA-Z]+[0-9]{2,32})$/;
return executeExp(pattern, strValue);
}
/**
* 判断是否为中文
...
* @returns {boolean}
*/
function isNumericD(strValue) {
    if (isEmpty(strValue))
        return false;
    var pattern = /^([0-9]\d*(\.\d*[0-9])?)|(0\.\d*[0-9])$/;
    return executeExp(pattern, strValue);
  }
/**
* 判断是否为浮点数（不带正负号）
...
var validatorUtils= {
      ...
      isEmail:isEmail,
      isMoney:isMoney,
      isEnNumericLimit:isEnNumericLimit,
      isNumeric:isNumeric,
      isNumericD:isNumericD,
      ...
      isEnNumerFirstEn:isEnNumerFirstEn
  };
