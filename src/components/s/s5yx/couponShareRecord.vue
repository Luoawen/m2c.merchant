      ...
      <el-col :span="24" class="">
      	...
      	<div class="table">
					<el-table :data="send_list" slot="empty" stripe style="width: 100%; margin-top: 20px">
							...
							<el-table-column prop="createdDate" label="发送时间" width="250">
							</el-table-column>
						</el-table>
      	</div>
      	<div class="fr mt10">
   ...
   methods: {
		...
		get_send () {
			...
			if (that.time !== null && that.time !== '') {
				that.params.start_date = that.date_format(new Date(that.time[0]),"yyyy-MM-dd")
				that.params.end_date = that.date_format(new Date(that.time[1]),"yyyy-MM-dd")
			}
