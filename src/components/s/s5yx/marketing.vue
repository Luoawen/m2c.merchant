<template>
  <div class="marketingFullCutbox">
        ...
        <div class="base rule clear">
          ...
          <div class="range col-xs-6">
            <div>
              ...
              <a v-show="rang_type_show.shop" @click="openShopChoose">点我选择商家</a>
              <a v-show="rang_type_show.classify" @click="openClassifyChoose">点我选择品类</a>
            ...
            <div class="classify" v-show="rang_type_show.classify" >
              <p>已选<span>{{params.goods_classifys.length}}</span>个品类</p>
              <ul>
              ...
              <div class="more">
                <a @click="openClassifyAll()" v-show="params.goods_classifys.length > 0">查看全部品类></a>
              </div>
            </div>
              ...
              <div>
                ...
                <ul>
                  <li v-for="(shop, index) in removeShopList">
                    <div>{{shop.dealerName}}</div>
                    <i class="icon_dele" @click="deleteRemoveShop(index, shop)"></i>
                  </li>
                </ul>
                <div class="more">
                  <a @click="shop_remove_all_show = true" v-show="removeShopList.length > 0">查看全部商家></a>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="info-content">
        ...
        <div>
          ...
          <div class="fl" style="margin-right:35px;">
            <span class="color_g ">平台承担：</span>
            <input type="number" class="form-control wid70 form-cd" v-model="params.cost_json.platform" @blur="formValidator(7)"/>
            <span>%</span>
          </div>
          <div>
            <span class="color_g">商家承担：</span>
            <input type="number" class="form-control wid70 form-cd"  v-model="params.cost_json.dealer" @blur="formValidator(8)"/>
            <span>%</span>
          </div>
        </div>
        <p style="margin-top:20px;margin-bottom:20px;">
          <a @click="openShopCostChoose()">单独成本设置</a>
        </p>
        <!-- 选择商家的成本设置 -->
        <!--<div class="cb-set">-->
          <!--<div class="tit" v-show="dealerCostList.length > 0">商家单独成本设置</div>-->
          <!--<div class="set" v-for="(cost, index) in dealerCostList">-->
            <!--<span>{{index + 1}}.</span>-->
            <!--&lt;!&ndash;<span>{{cost.shopName}}</span>&ndash;&gt;-->
            <!--<span>平台承担:{{cost.platform}}%，</span>-->
            <!--<span>商家承担:{{cost.dealer}}%  </span>-->
          <!--</div>-->
          <!--<i class="icon_dele"></i>-->
        <!--</div>-->
      </div>
      ...
      <div class="modal-dialog frame" style="background:#fff;">
        <div class="frame_total ">
          ...
          <div class="search">
            ...
            <div class="clear">
              <input class="input form-control fl" type="text" placeholder="请输入商品标题或者编号进行搜索" v-model="goods_query_item.condition" />
              <button class="sort fl" @click="goodsChoose">搜索</button>
              <div class="right">
                <a  class="a_btn " style="color:rgba(17,210,235,1);" @click="openGoodsClassifyChoose()">商品品类选择</a>
                <a  class="a_btn " @click="openGoodsShopChoose()">商家筛选</a>
              </div>
            </div>
          </div>
          <div class="goods_body">
            <div class="merchant fl" v-for="(goods, index) in goodsResult.content" :key="index" >
              <div @click="openGoodsSku(goods)">
                ...
                <div class="fc" v-show="goods.isChoosed">
                </div>
                <div class="fcimg" v-show="goods.isChoosed"></div>
              </div>
            </div>
            <div class="page">
              <button>上一页</button>
    ...
    <!--作用范围商品筛选弹框e-->
    <!--作用范围商品规格选择弹窗s-->
    <div :class='["frame_layer",goods_sku_show?"frame_layer_show":""]' style="z-index:99999;" >
      <div class="specification_container modal-dialog">
          ...
          <div class="specification_tab fl">
            ...
            <table class="spec_table">
              ...
              <tr v-for="sku in goodsInfo.goodsSkuList">
                ...
                <td><input class="form-control set_num fl ml10" v-model="sku.goodsSkuNum"/></td>
              </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="specification_footer">
          ...
          <button type="button" class="btn cancel" @click="goodsSkuChoose(goodsInfo)">确认</button>
        </div>
      </div>
    </div>
    <!--作用范围商品规格选择弹窗e-->
    ...
    <div class="modal fade frame_layer01" id="choose_shop" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="z-index: 1051;">
      <div class="modal-dialog frame" style="background:#fff;">
        ...
        <div class="frame_total ">
          ...
          <div class="tc_shop">
            <div>已选择<span style="color:red;">{{chooseShopList.length}}<span>个</span></span>商家</div>
            <div class="tc_shop_box">
              <span class="tc_shop_span"  v-for="(choose,index) in chooseShopList">
                <span style="position: relative;display:inline-block;width:100%;height:100%;">
                  <span>{{choose.shopName}}</span>
                  <i class="i_delet" @click="deleteShop(index, choose)"></i>
                </span>
              </span>
            </div>
          </div>
          ...
          <div class="goods_body">
            <div class="merchant fl" v-for="(shop, index) in shopResult.content" :key="index" >
      ...
      formValidator (flag) {
        ...
        if (flag == 0) {
          ...
          if (that.dealerCostList.length > 0) {
            if (that.params.cost_json.platform == '' || that.params.cost_json.dealer == '') {
              alert('请填写通用成本设置')
              return false
            }
            for (var i = 0; i < that.dealerCostList.length; i++) {
              ...
              if (that.dealerCostList[i].platform != '' && that.dealerCostList[i].dealer != '' && parseInt(that.dealerCostList[i].platform) + parseInt(that.dealerCostList[i].dealer) != 100 ) {
...
//          url: that.localbase + 'm2c.scm/dealerclassify/sys/firstClassify',
      ...
      addRemoveShop (shop) {
        ...
        for (var i = 0; i < that.shopResult.content.length; i++) {
          if (that.shopResult.content[i].isRemoved == 1) {
            that.removeShopList.push(that.shopResult.content[i])
          }
        }
      },
      cancelRemove(){
      ...
      openGoodsExcahnge () {
        var that = this
        that.$('#goods_exchange_dialog').modal('show')
        that.goods_query_item.goodsClassifyId = ''
        that.goods_query_item.condition = ''
        that.goods_query_item.dealerId = ''
        that.goods_query_item.pageNum = 1
      ...
      addExchangeGoods (exchange) {
        ...
        for (var i = 0; i < that.goodsResult.content.length; i++) {
          if (that.goodsResult.content[i].isExchange == 1) {
            that.exchangeGoodsList.push(that.goodsResult.content[i])
          }
        }
      },
